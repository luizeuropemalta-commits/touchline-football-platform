-- QA-only conservative rollback for the unapplied/empty 047 goal-family move.
-- Refuses to proceed after any 047 Hat-trick draft/job has been materialised.
...
do $$
begin
  if exists (select 1 from public.touchline_social_publication_drafts
    where content_type='HAT_TRICK_HERO' and event_provider_id is not null)
    or exists (select 1 from public.touchline_social_confirmed_event_generation_jobs
      where content_type='HAT_TRICK_HERO' or template_version='touchline-goal-event-feed-v1') then
    raise exception 'TL_SOCIAL_047_ROLLBACK_NONEMPTY';
  end if;
end
$$;
...
begin
  foreach v_name in array array[
    ...
    if pg_catalog.to_regprocedure(v_name) is null then
      raise exception 'TL_SOCIAL_047_ROLLBACK_FUNCTION_MISSING_%', v_name;
    end if;
    ...
    if v_definition = v_original then
      raise exception 'TL_SOCIAL_047_ROLLBACK_FUNCTION_DRIFT_%', v_name;
    end if;
    if pg_catalog.position(
         '(''GOAL_CONFIRMED'', ''RED_CARD_CONFIRMED'')' in v_definition
       ) = 0
       or pg_catalog.position(
         '(''GOAL_CONFIRMED'', ''RED_CARD_CONFIRMED'', ''HAT_TRICK_HERO'')' in v_definition
       ) > 0
       or (v_name like '%create_draft%' and (
         pg_catalog.position('INSTAGRAM_FEED' in v_definition) > 0
         or pg_catalog.position('or p_draft ->> ''placement'' <> ''INSTAGRAM_STORY''' in v_definition) = 0
       ))
       or (v_name like '%enqueue_job%' and (
         pg_catalog.position('touchline-goal-confirmed-story-v1' in v_definition) = 0
         or pg_catalog.position('touchline-hat-trick-feed-v1' in v_definition) > 0
       )) then
      raise exception 'TL_SOCIAL_047_ROLLBACK_FUNCTION_PATCH_INCOMPLETE_%', v_name;
    end if;
...
alter table public.touchline_social_publication_drafts
  drop constraint touchline_social_drafts_047_relation_check,
  drop constraint touchline_social_drafts_047_placement_check,
  drop constraint touchline_social_drafts_047_render_path_check;
...
begin
  ...
  loop
    execute pg_catalog.format('alter table public.touchline_social_confirmed_event_generation_jobs drop constraint %I', v_constraint.conname);
  end loop;
end
$$;

alter table public.touchline_social_confirmed_event_generation_jobs
  add constraint touchline_social_confirmed_event_jobs_043_content_type_check
  check (content_type in ('GOAL_CONFIRMED','RED_CARD_CONFIRMED')) not valid;

drop trigger if exists touchline_social_047_block_hat_trick_in_044
  on public.touchline_social_ranking_generation_jobs;
drop function if exists public.touchline_social_047_block_hat_trick_in_044();
...
begin
  if (new.artwork_approval_state = 'APPROVED' and old.artwork_approval_state is distinct from new.artwork_approval_state)
    ...
    if new.content_type in ('GAMEWEEK_RANKING_PREVIEW','GAMEWEEK_RANKING_FINAL','PLAYER_DUEL','GAMEWEEK_HERO','TOP_PERFORMER','HAT_TRICK_HERO') then
