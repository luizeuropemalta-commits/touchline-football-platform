-- QA-only published facts for native ClubHub/ClubOwner feed rendering.
-- Facts are optional and absent facts must be represented as unavailable by
-- the application; this migration never derives facts from social artwork.
begin;
...
begin
  if pg_catalog.to_regclass('public.touchline_club_social_posts') is null
     or pg_catalog.to_regprocedure('public.touchline_social_045_read_feed(text,integer,timestamptz,uuid)') is null then
    raise exception 'TL_SOCIAL_052_SCHEMA_PRECONDITION_FAILED';
  end if;
end $$;

alter table public.touchline_club_social_posts
  add column if not exists published_presentation jsonb;

alter table public.touchline_club_social_posts
  drop constraint if exists touchline_club_social_posts_published_presentation_check;
alter table public.touchline_club_social_posts
...
begin
  ...
  select coalesce(pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object(
    ...
    'presentation',feed.published_presentation
