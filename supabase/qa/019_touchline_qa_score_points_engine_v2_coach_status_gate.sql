...
create or replace function public.touchline_is_scoreable_fixture_status(p_status text)
returns boolean
language sql
immutable
parallel safe
...
as $$
  select regexp_replace(
    ...
    'live', 'in play', 'in progress', '1st half', '2nd half', 'half time', 'ht',
    'extra time', 'penalties', 'penalties finished',
    'ft', 'aet', 'pen', 'finished', 'full time', 'after extra time', 'after penalties'
  ]::text[]);
$$;
...
begin
  ...
  loop
    ...
    v_settlement := case
      when lower(trim(coalesce(v_record.status,''))) in (
        'ft','aet','pen','finished','full time','after extra time','after penalties'
      ) then 'final'
      else 'provisional'
    end;
...
revoke all on function public.touchline_is_scoreable_fixture_status(text)
...
comment on function public.touchline_is_scoreable_fixture_status(text) is
  'Server-only V2 gate: scheduled fixtures never create provisional or final coach points.';
