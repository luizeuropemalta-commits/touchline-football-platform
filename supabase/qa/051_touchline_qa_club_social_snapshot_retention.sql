-- QA-only immutable snapshot retention for ClubHub, ClubOwner and Share.
-- Source currentness remains mandatory before approval and fanout. Once a
-- canonical post is published, readers retain its exact approved snapshot
-- until expires_at while continuing to enforce every immutable hash fence.
...
begin
  if pg_catalog.to_regclass('public.touchline_club_social_posts') is null
    ...
    raise exception 'TL_SOCIAL_051_SCHEMA_PRECONDITION_FAILED';
  end if;
end
$$;
