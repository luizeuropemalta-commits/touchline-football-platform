-- QA-only rollback for 20260915103000.  It restores the immediately preceding
-- five-minute opening policy while retaining the finalized_at trigger and its
-- fail-closed predecessor state machine.
  ...
  with round_facts as (
    ...
    select
      ...
      case
        ...
        when previous_round_all_final and previous_round_completed_at is not null
          and previous_round_completed_at + interval '5 minutes' < locks_at
          then previous_round_completed_at + interval '5 minutes'
    ...
    select
      ...
      case
        when round_sequence > 1 and clock_timestamp() < locks_at
          and (not coalesce(previous_round_all_final, false)
          or previous_round_completed_at is null
          or previous_round_completed_at + interval '5 minutes' >= locks_at) then 'UPCOMING'
