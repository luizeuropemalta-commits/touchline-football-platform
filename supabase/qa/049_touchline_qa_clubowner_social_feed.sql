-- QA-only ClubOwner Timeline reader. It references the same canonical 045
-- post and immutable media as ClubHub, without duplicating bytes or enabling
-- any external social destination.
...
begin
  if pg_catalog.to_regclass('public.touchline_club_social_posts') is null
     ...
     or pg_catalog.to_regprocedure('public.touchline_social_source_revision_is_current(jsonb,text)') is null then
    raise exception 'TL_SOCIAL_049_SCHEMA_PRECONDITION_FAILED';
  end if;
