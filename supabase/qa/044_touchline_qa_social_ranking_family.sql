-- QA-only ranking, duel and verified hero family extending frozen 039-043.
-- Generates approval-only Feed drafts; no outbound delivery exists here.
...
begin
  if pg_catalog.to_regclass('public.touchline_social_publication_drafts') is null
     ...
     or pg_catalog.to_regclass('public.touchline_social_confirmed_event_executor_cycles') is null
     or pg_catalog.to_regclass('public.touchline_social_confirmed_event_generation_jobs') is null
     or pg_catalog.to_regclass('public.touchline_social_ranking_executor_cycles') is not null
     or pg_catalog.to_regclass('public.touchline_social_ranking_generation_jobs') is not null
     or pg_catalog.to_regclass('public.touchline_social_ranking_review_intents') is not null then
    raise exception 'TL_SOCIAL_RANKING_044_SCHEMA_PRECONDITION_FAILED';
  end if;
end
$$;
...
alter table public.touchline_social_source_revisions
  ...
  add constraint touchline_social_source_revisions_044_source_key_check check (
...
-- 039/040/041/042/043 remain the prior content authorities. 044 extends
-- only the shared draft envelope and approval dispatcher for ranking products.
do $$
declare
  v_count integer := 0;
  v_constraint record;
...
begin
  ...
  where conrelid = 'public.touchline_social_publication_drafts'::regclass
    and conname = any(array[
      'touchline_social_drafts_043_content_type_check',
      'touchline_social_drafts_043_relation_check',
      'touchline_social_drafts_043_input_check',
      'touchline_social_drafts_043_placement_check',
      'touchline_social_drafts_043_render_path_check'
    ]::text[]);
  if v_count <> 5 then
    raise exception 'TL_SOCIAL_RANKING_044_DRAFT_CONSTRAINT_DRIFT_%', v_count;
  end if;
  for v_constraint in
  ...
  if v_publication_constraints <> 1 or v_object_constraints <> 1 then
    raise exception 'TL_SOCIAL_RANKING_044_IDENTITY_CONSTRAINT_DRIFT_%_%',
...
alter table public.touchline_social_publication_drafts
  add column scope_provider_id text,
  add column subject_player_provider_id text,
  add constraint touchline_social_drafts_044_content_type_check
  ...
  add constraint touchline_social_drafts_044_scope_id_check
    check (scope_provider_id is null or scope_provider_id ~ '^[1-9][0-9]{0,19}$'),
  add constraint touchline_social_drafts_044_subject_player_id_check
    check (subject_player_provider_id is null or subject_player_provider_id ~ '^[1-9][0-9]{0,19}$'),
  add constraint touchline_social_drafts_044_relation_check check (
    ...
    or (content_type in ('TOP_PERFORMER','HAT_TRICK_HERO')
      and team_provider_id is null and event_provider_id is null
      and scope_provider_id is null and subject_player_provider_id is not null)
  ),
  add constraint touchline_social_drafts_044_input_check
    check (input_checksum = source_checksum),
  add constraint touchline_social_drafts_044_placement_check check (
    (content_type in (
      'LINEUP','MATCH_PREVIEW','FULL_TIME','GAMEWEEK_RANKING_PREVIEW','GAMEWEEK_RANKING_FINAL',
      'PLAYER_DUEL','GAMEWEEK_HERO','TOP_PERFORMER','HAT_TRICK_HERO'
  ...
  add constraint touchline_social_drafts_044_render_path_check check (
    render_path = case content_type
      ...
      when 'TOP_PERFORMER' then '/visual-qa/social-ranking?contentType=' || content_type
        ...
        || '&locale=' || locale || '&revision=' || revision::text
      when 'HAT_TRICK_HERO' then '/visual-qa/social-ranking?contentType=' || content_type
        || '&fixtureId=' || fixture_provider_id || '&playerId=' || subject_player_provider_id
        || '&locale=' || locale || '&revision=' || revision::text
    end
  ),
  add constraint touchline_social_drafts_044_publication_key_check check (
    publication_key = 'instagram:' || placement || ':' || content_type || ':'
      || coalesce(scope_provider_id, fixture_provider_id) || ':'
      || coalesce(subject_player_provider_id, event_provider_id, team_provider_id,
        case when scope_provider_id is not null then 'gameweek' else 'fixture' end) || ':'
      || locale || ':tv=' || template_version || ':sv=' || source_version || ':r=' || revision::text
  ),
  add constraint touchline_social_drafts_044_object_key_check check (
    artifact_storage_key = 'instagram/' || lower(placement) || '/' || lower(content_type) || '/'
      || coalesce(scope_provider_id, fixture_provider_id) || '/'
      || coalesce(subject_player_provider_id, event_provider_id, team_provider_id,
        case when scope_provider_id is not null then 'gameweek' else 'fixture' end) || '/'
      || locale || '/tv=' || template_version || '/sv=' || source_version || '/r=' || revision::text || '/'
...
create or replace function public.touchline_social_044_guard_draft_identity()
returns trigger language plpgsql set search_path = '' as $$
begin
  if row(new.scope_provider_id, new.subject_player_provider_id)
     is distinct from row(old.scope_provider_id, old.subject_player_provider_id) then
    raise exception 'TL_SOCIAL_RANKING_DRAFT_SCOPE_IMMUTABLE';
  end if;
  return new;
end
$$;
drop trigger if exists touchline_social_044_draft_identity_guard on public.touchline_social_publication_drafts;
create trigger touchline_social_044_draft_identity_guard
before update on public.touchline_social_publication_drafts for each row
execute function public.touchline_social_044_guard_draft_identity();

create table public.touchline_social_ranking_executor_cycles (
...
create table public.touchline_social_ranking_generation_jobs (
  id uuid primary key default gen_random_uuid(),
  fixture_provider_id text not null check (fixture_provider_id ~ '^[1-9][0-9]{0,19}$'),
  scope_provider_id text check (scope_provider_id is null or scope_provider_id ~ '^[1-9][0-9]{0,19}$'),
  subject_player_provider_id text check (
    subject_player_provider_id is null or subject_player_provider_id ~ '^[1-9][0-9]{0,19}$'
  ),
  content_type text not null check (content_type in (
    'GAMEWEEK_RANKING_PREVIEW','GAMEWEEK_RANKING_FINAL','PLAYER_DUEL',
    'GAMEWEEK_HERO','TOP_PERFORMER','HAT_TRICK_HERO'
  )),
  template_version text not null check (template_version = 'touchline-social-ranking-feed-v1'),
  ...
  unique nulls not distinct (
    fixture_provider_id, scope_provider_id, subject_player_provider_id,
    content_type, template_version, input_checksum, source_revision_checksum
  ),
  check (
    (content_type in ('GAMEWEEK_RANKING_PREVIEW','GAMEWEEK_RANKING_FINAL')
      and scope_provider_id is not null and subject_player_provider_id is null)
    or (content_type = 'GAMEWEEK_HERO' and scope_provider_id is not null and subject_player_provider_id is not null)
    or (content_type = 'PLAYER_DUEL' and scope_provider_id is null and subject_player_provider_id is null)
    or (content_type in ('TOP_PERFORMER','HAT_TRICK_HERO')
      and scope_provider_id is null and subject_player_provider_id is not null)
  ),
  check (
    (content_type in ('GAMEWEEK_RANKING_PREVIEW','PLAYER_DUEL')
      and first_observed_at <= starts_at)
    or (content_type in ('GAMEWEEK_RANKING_FINAL','GAMEWEEK_HERO','TOP_PERFORMER','HAT_TRICK_HERO')
      and first_observed_at >= starts_at)
  ),
  check (
...
create table public.touchline_social_ranking_review_intents (
  id uuid primary key default gen_random_uuid(),
...
create index touchline_social_ranking_jobs_ready_idx
  on public.touchline_social_ranking_generation_jobs (next_eligible_at, starts_at, id)
  where job_state in ('PENDING', 'RETRY_WAIT');

create or replace function public.touchline_social_044_create_draft(p_draft jsonb)
returns jsonb language plpgsql security definer set search_path = '' as $$
declare v_id uuid; v_existing public.touchline_social_publication_drafts%rowtype;
  ...
  v_content_type text := coalesce(p_draft ->> 'content_type', '');
  v_fixture_provider_id text := p_draft ->> 'fixture_provider_id';
  v_scope_provider_id text := nullif(p_draft ->> 'scope_provider_id', '');
  v_subject_player_provider_id text := nullif(p_draft ->> 'subject_player_provider_id', '');
begin
  if jsonb_typeof(p_draft) <> 'object' or v_publication_key = ''
     ...
     or coalesce(v_fixture_provider_id, '') !~ '^[1-9][0-9]{0,19}$'
     or p_draft ->> 'team_provider_id' is not null
     or p_draft ->> 'event_provider_id' is not null
     or p_draft ->> 'placement' <> 'INSTAGRAM_FEED'
     or p_draft ->> 'template_version' <> 'touchline-social-ranking-feed-v1'
     or (
       (v_content_type in ('GAMEWEEK_RANKING_PREVIEW','GAMEWEEK_RANKING_FINAL')
         and (coalesce(v_scope_provider_id, '') !~ '^[1-9][0-9]{0,19}$'
           or v_subject_player_provider_id is not null))
       or (v_content_type = 'GAMEWEEK_HERO'
         and (coalesce(v_scope_provider_id, '') !~ '^[1-9][0-9]{0,19}$'
           or coalesce(v_subject_player_provider_id, '') !~ '^[1-9][0-9]{0,19}$'))
       or (v_content_type = 'PLAYER_DUEL'
         and (v_scope_provider_id is not null or v_subject_player_provider_id is not null))
       or (v_content_type in ('TOP_PERFORMER','HAT_TRICK_HERO')
         and (v_scope_provider_id is not null
           or coalesce(v_subject_player_provider_id, '') !~ '^[1-9][0-9]{0,19}$'))
     )
    ...
    raise exception 'TL_SOCIAL_RANKING_DRAFT_CREATE_INPUT_INVALID';
  end if;
  perform pg_catalog.pg_advisory_xact_lock_shared(
    pg_catalog.hashtextextended('touchline-social-source-revision', 0));
  perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended(
    'touchline-social-044-generation:' || v_fixture_provider_id || ':'
      || coalesce(v_scope_provider_id, '0') || ':' || coalesce(v_subject_player_provider_id, '0') || ':'
      || v_content_type || ':'
  ...
  insert into public.touchline_social_publication_drafts(
    publication_key, fixture_provider_id, team_provider_id, event_provider_id,
    scope_provider_id, subject_player_provider_id, content_type,
    ...
    v_publication_key, v_fixture_provider_id, null, null,
    v_scope_provider_id, v_subject_player_provider_id, v_content_type, p_draft ->> 'placement',
  ...
  if v_existing.id is null
     or v_existing.fixture_provider_id <> v_fixture_provider_id
     or v_existing.event_provider_id is not null
     or v_existing.scope_provider_id is distinct from v_scope_provider_id
     or v_existing.subject_player_provider_id is distinct from v_subject_player_provider_id
    ...
    raise exception 'TL_SOCIAL_RANKING_DRAFT_IDENTITY_CONFLICT';
  end if;
  return jsonb_build_object('ok', true, 'draftId', v_existing.id, 'outcome', 'noop_existing');
end
$$;

create or replace function public.touchline_social_044_guard_cycle_mutation()
returns trigger language plpgsql set search_path = '' as $$
declare v_transition text := coalesce(current_setting('touchline.social_ranking_transition', true), '');
begin
  if tg_op = 'DELETE' or v_transition not in ('claim_cycle', 'renew_cycle', 'complete_cycle') then
    raise exception 'TL_SOCIAL_RANKING_CYCLE_RPC_REQUIRED';
  end if;
  if tg_op = 'UPDATE' and new.component is distinct from old.component then
    raise exception 'TL_SOCIAL_RANKING_CYCLE_IDENTITY_IMMUTABLE';
  end if;
  new.updated_at := clock_timestamp(); return new;
end
$$;

create trigger touchline_social_044_cycle_guard before insert or update or delete
on public.touchline_social_ranking_executor_cycles for each row
execute function public.touchline_social_044_guard_cycle_mutation();

create or replace function public.touchline_social_044_guard_job_mutation()
returns trigger language plpgsql set search_path = '' as $$
declare v_transition text := coalesce(current_setting('touchline.social_ranking_transition', true), '');
begin
  if tg_op = 'DELETE' or v_transition not in ('enqueue', 'claim_job', 'renew_job', 'complete_job', 'recover_job') then
    raise exception 'TL_SOCIAL_RANKING_JOB_RPC_REQUIRED';
  end if;
  if tg_op = 'UPDATE' and row(
    new.fixture_provider_id, new.scope_provider_id, new.subject_player_provider_id,
    new.content_type, new.template_version, new.first_observed_at,
    ...
    old.fixture_provider_id, old.scope_provider_id, old.subject_player_provider_id,
    old.content_type, old.template_version, old.first_observed_at,
  ...
  ) then raise exception 'TL_SOCIAL_RANKING_JOB_IDENTITY_IMMUTABLE'; end if;
  new.updated_at := clock_timestamp(); return new;
end
$$;

create trigger touchline_social_044_job_guard before insert or update or delete
on public.touchline_social_ranking_generation_jobs for each row
execute function public.touchline_social_044_guard_job_mutation();

create or replace function public.touchline_social_044_claim_cycle(p_component text)
returns jsonb language plpgsql security definer set search_path = '' as $$
declare v_cycle public.touchline_social_ranking_executor_cycles%rowtype;
...
begin
  if coalesce(p_component, '') not in ('SCHEDULER', 'RUNNER') then
    raise exception 'TL_SOCIAL_RANKING_CYCLE_CLAIM_INVALID'; end if;
  perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('touchline-social-044-executor:' || p_component, 0));
  select * into v_cycle from public.touchline_social_ranking_executor_cycles
    where component = p_component for update;
  if v_cycle.component is null then
    perform set_config('touchline.social_ranking_transition', 'claim_cycle', true);
    insert into public.touchline_social_ranking_executor_cycles(component) values (p_component)
  ...
  if v_cycle.lease_token is not null then
    ...
    perform set_config('touchline.social_ranking_transition', 'complete_cycle', true);
    update public.touchline_social_ranking_executor_cycles set
...
create or replace function public.touchline_social_044_renew_cycle(p_component text, p_lease_token uuid)
returns jsonb language plpgsql security definer set search_path = '' as $$
declare v_cycle public.touchline_social_ranking_executor_cycles%rowtype; v_now timestamptz := clock_timestamp();
begin
  if coalesce(p_component, '') not in ('SCHEDULER', 'RUNNER') or p_lease_token is null then
    raise exception 'TL_SOCIAL_RANKING_CYCLE_RENEW_INVALID'; end if;
  perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('touchline-social-044-executor:' || p_component, 0));
  select * into v_cycle from public.touchline_social_ranking_executor_cycles
    where component = p_component for update;
  if v_cycle.lease_token is distinct from p_lease_token or v_cycle.lease_expires_at <= v_now then
    raise exception 'TL_SOCIAL_RANKING_CYCLE_LEASE_INVALID'; end if;
  perform set_config('touchline.social_ranking_transition', 'renew_cycle', true);
  update public.touchline_social_ranking_executor_cycles set
...
create or replace function public.touchline_social_044_complete_cycle(
...
begin
  if coalesce(p_component, '') not in ('SCHEDULER', 'RUNNER') or p_lease_token is null
    ...
    raise exception 'TL_SOCIAL_RANKING_CYCLE_COMPLETE_INVALID'; end if;
  perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('touchline-social-044-executor:' || p_component, 0));
  select * into v_cycle from public.touchline_social_ranking_executor_cycles
    where component = p_component for update;
  if v_cycle.lease_token is distinct from p_lease_token or v_cycle.lease_expires_at <= v_now then
...
create or replace function public.touchline_social_044_enqueue_job(
  p_scheduler_lease_token uuid, p_fixture_provider_id text, p_scope_provider_id text,
  p_subject_player_provider_id text,
...
returns jsonb language plpgsql security definer set search_path = '' as $$
declare v_cycle public.touchline_social_ranking_executor_cycles%rowtype;
  v_job public.touchline_social_ranking_generation_jobs%rowtype;
  v_first_observed_at timestamptz;
begin
  if p_scheduler_lease_token is null or coalesce(p_fixture_provider_id, '') !~ '^[1-9][0-9]{0,19}$'
     or coalesce(p_content_type, '') not in (
       'GAMEWEEK_RANKING_PREVIEW','GAMEWEEK_RANKING_FINAL','PLAYER_DUEL',
       'GAMEWEEK_HERO','TOP_PERFORMER','HAT_TRICK_HERO'
     )
     or p_template_version <> 'touchline-social-ranking-feed-v1'
     or (
       (p_content_type in ('GAMEWEEK_RANKING_PREVIEW','GAMEWEEK_RANKING_FINAL')
         and (coalesce(p_scope_provider_id, '') !~ '^[1-9][0-9]{0,19}$'
           or p_subject_player_provider_id is not null))
       or (p_content_type = 'GAMEWEEK_HERO'
         and (coalesce(p_scope_provider_id, '') !~ '^[1-9][0-9]{0,19}$'
           or coalesce(p_subject_player_provider_id, '') !~ '^[1-9][0-9]{0,19}$'))
       or (p_content_type = 'PLAYER_DUEL'
         and (p_scope_provider_id is not null or p_subject_player_provider_id is not null))
       or (p_content_type in ('TOP_PERFORMER','HAT_TRICK_HERO')
         and (p_scope_provider_id is not null
           or coalesce(p_subject_player_provider_id, '') !~ '^[1-9][0-9]{0,19}$'))
     )
     or p_first_observed_at is null or p_starts_at is null
     or (p_content_type in ('GAMEWEEK_RANKING_PREVIEW','PLAYER_DUEL')
       and p_first_observed_at > p_starts_at)
     or (p_content_type in ('GAMEWEEK_RANKING_FINAL','GAMEWEEK_HERO','TOP_PERFORMER','HAT_TRICK_HERO')
       and p_first_observed_at < p_starts_at)
    ...
    raise exception 'TL_SOCIAL_RANKING_JOB_INPUT_INVALID'; end if;
  perform pg_catalog.pg_advisory_xact_lock_shared(pg_catalog.hashtextextended('touchline-social-source-revision', 0));
  perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended(
    'touchline-social-044-generation:' || p_fixture_provider_id || ':'
      || coalesce(p_scope_provider_id, '0') || ':' || coalesce(p_subject_player_provider_id, '0') || ':'
      || p_content_type || ':' || p_template_version, 0));
  perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('touchline-social-044-executor:SCHEDULER', 0));
  select * into v_cycle from public.touchline_social_ranking_executor_cycles
  ...
  if v_cycle.lease_token is distinct from p_scheduler_lease_token or v_cycle.lease_expires_at <= clock_timestamp() then
    raise exception 'TL_SOCIAL_RANKING_SCHEDULER_LEASE_INVALID'; end if;
  ...
  from public.touchline_social_ranking_generation_jobs
  where fixture_provider_id = p_fixture_provider_id
    and scope_provider_id is not distinct from p_scope_provider_id
    and subject_player_provider_id is not distinct from p_subject_player_provider_id
    and content_type = p_content_type
    and template_version = p_template_version;
  perform set_config('touchline.social_ranking_transition', 'recover_job', true);
  update public.touchline_social_ranking_generation_jobs set
  ...
  perform set_config('touchline.social_ranking_transition', 'enqueue', true);
  insert into public.touchline_social_ranking_generation_jobs(
    fixture_provider_id, scope_provider_id, subject_player_provider_id,
    content_type, template_version, first_observed_at, starts_at,
    input_checksum, source_revision_manifest, source_revision_checksum,
    job_state, reason_code, next_eligible_at
  ) values (
    p_fixture_provider_id, p_scope_provider_id, p_subject_player_provider_id,
    p_content_type, p_template_version, v_first_observed_at, p_starts_at,
  ...
  return jsonb_build_object('ok', true, 'outcome', case when v_job.attempt_count = 0 then 'queued' else 'noop_existing' end,
    'jobId', v_job.id, 'state', v_job.job_state, 'scopeId', v_job.scope_provider_id,
    'playerId', v_job.subject_player_provider_id, 'contentType', v_job.content_type,
    'templateVersion', v_job.template_version);
end
$$;

create or replace function public.touchline_social_044_claim_job(p_runner_lease_token uuid)
returns jsonb language plpgsql security definer set search_path = '' as $$
declare v_cycle public.touchline_social_ranking_executor_cycles%rowtype;
  v_job public.touchline_social_ranking_generation_jobs%rowtype;
  v_now timestamptz := clock_timestamp(); v_token uuid := gen_random_uuid();
begin
  if p_runner_lease_token is null then raise exception 'TL_SOCIAL_RANKING_JOB_CLAIM_INVALID'; end if;
  perform pg_catalog.pg_advisory_xact_lock_shared(pg_catalog.hashtextextended('touchline-social-source-revision', 0));
  perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('touchline-social-044-executor:RUNNER', 0));
  select * into v_cycle from public.touchline_social_ranking_executor_cycles
  ...
  if v_cycle.lease_token is distinct from p_runner_lease_token or v_cycle.lease_expires_at <= v_now then
    raise exception 'TL_SOCIAL_RANKING_RUNNER_LEASE_INVALID'; end if;
  perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('touchline-social-044-job-queue', 0));
  perform set_config('touchline.social_ranking_transition', 'recover_job', true);
  update public.touchline_social_ranking_generation_jobs set
  ...
  select * into v_job from public.touchline_social_ranking_generation_jobs
  ...
  perform set_config('touchline.social_ranking_transition', 'claim_job', true);
  update public.touchline_social_ranking_generation_jobs set
  ...
  return jsonb_build_object('ok', true, 'outcome', 'claimed', 'jobId', v_job.id, 'leaseToken', v_token,
    'fixtureId', v_job.fixture_provider_id, 'scopeId', v_job.scope_provider_id,
    'playerId', v_job.subject_player_provider_id, 'contentType', v_job.content_type,
...
create or replace function public.touchline_social_044_renew_job(
  p_runner_lease_token uuid, p_job_id uuid, p_job_lease_token uuid
)
returns jsonb language plpgsql security definer set search_path = '' as $$
declare v_cycle public.touchline_social_ranking_executor_cycles%rowtype;
  v_job public.touchline_social_ranking_generation_jobs%rowtype; v_now timestamptz := clock_timestamp();
begin
  perform pg_catalog.pg_advisory_xact_lock_shared(pg_catalog.hashtextextended('touchline-social-source-revision', 0));
  perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('touchline-social-044-executor:RUNNER', 0));
  select * into v_cycle from public.touchline_social_ranking_executor_cycles where component = 'RUNNER' for update;
  if v_cycle.lease_token is distinct from p_runner_lease_token or v_cycle.lease_expires_at <= v_now then
    raise exception 'TL_SOCIAL_RANKING_RUNNER_LEASE_INVALID'; end if;
  select * into v_job from public.touchline_social_ranking_generation_jobs where id = p_job_id for update;
  if v_job.job_state <> 'RUNNING' or v_job.lease_token is distinct from p_job_lease_token
    ...
    raise exception 'TL_SOCIAL_RANKING_JOB_LEASE_INVALID'; end if;
  perform set_config('touchline.social_ranking_transition', 'renew_job', true);
  update public.touchline_social_ranking_generation_jobs set
...
create or replace function public.touchline_social_044_complete_job(
  p_runner_lease_token uuid, p_job_id uuid, p_job_lease_token uuid,
...
begin
  if p_runner_lease_token is null or p_job_id is null or p_job_lease_token is null
    ...
    raise exception 'TL_SOCIAL_RANKING_JOB_COMPLETE_INVALID'; end if;
  perform pg_catalog.pg_advisory_xact_lock_shared(pg_catalog.hashtextextended('touchline-social-source-revision', 0));
  perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('touchline-social-044-executor:RUNNER', 0));
  select * into v_cycle from public.touchline_social_ranking_executor_cycles where component = 'RUNNER' for update;
  if v_cycle.lease_token is distinct from p_runner_lease_token or v_cycle.lease_expires_at <= v_now then
  ...
  select * into v_job from public.touchline_social_ranking_generation_jobs where id = p_job_id for update;
  if v_job.job_state <> 'RUNNING' or v_job.lease_token is distinct from p_job_lease_token
     or v_job.lease_expires_at <= v_now then raise exception 'TL_SOCIAL_RANKING_JOB_LEASE_INVALID'; end if;
  if p_outcome = 'COMPLETED' and (
    ...
    or not exists (
      ...
      where draft.id = p_generated_draft_id and draft.content_type = v_job.content_type
        ...
        and draft.event_provider_id is null
        and draft.scope_provider_id is not distinct from v_job.scope_provider_id
        and draft.subject_player_provider_id is not distinct from v_job.subject_player_provider_id
  ...
  ) then raise exception 'TL_SOCIAL_RANKING_JOB_DRAFT_MISMATCH'; end if;
  ...
  perform set_config('touchline.social_ranking_transition', 'complete_job', true);
  update public.touchline_social_ranking_generation_jobs set job_state = v_state,
...
create or replace function public.touchline_social_044_assert_approval_gate(p_draft_id uuid)
returns void language plpgsql security definer set search_path = '' as $$
declare v_draft public.touchline_social_publication_drafts%rowtype;
  v_cycle public.touchline_social_ranking_executor_cycles%rowtype;
  v_job public.touchline_social_ranking_generation_jobs%rowtype;
...
begin
  ...
  if v_draft.id is null or v_draft.content_type not in (
       'GAMEWEEK_RANKING_PREVIEW','GAMEWEEK_RANKING_FINAL','PLAYER_DUEL',
       'GAMEWEEK_HERO','TOP_PERFORMER','HAT_TRICK_HERO'
     )
     or v_draft.team_provider_id is not null or v_draft.event_provider_id is not null
     or (
       (v_draft.content_type in ('GAMEWEEK_RANKING_PREVIEW','GAMEWEEK_RANKING_FINAL')
         and (v_draft.scope_provider_id is null or v_draft.subject_player_provider_id is not null))
       or (v_draft.content_type = 'GAMEWEEK_HERO'
         and (v_draft.scope_provider_id is null or v_draft.subject_player_provider_id is null))
       or (v_draft.content_type = 'PLAYER_DUEL'
         and (v_draft.scope_provider_id is not null or v_draft.subject_player_provider_id is not null))
       or (v_draft.content_type in ('TOP_PERFORMER','HAT_TRICK_HERO')
         and (v_draft.scope_provider_id is not null or v_draft.subject_player_provider_id is null))
     )
     or not public.touchline_social_source_revision_is_current(
       v_draft.source_revision_manifest, v_draft.source_revision_checksum
     ) then raise exception 'TL_SOCIAL_RANKING_APPROVAL_DRAFT_INVALID'; end if;
  for v_cycle in select * from public.touchline_social_ranking_executor_cycles
  ...
  loop
    ...
    if v_cycle.lease_token is not null or v_cycle.last_outcome is distinct from 'SUCCESS'
      ...
      raise exception 'TL_SOCIAL_RANKING_APPROVAL_HEALTH_UNSAFE'; end if;
  end loop;
  if v_count <> 2 then raise exception 'TL_SOCIAL_RANKING_APPROVAL_HEALTH_UNSAFE'; end if;
  select * into v_job from public.touchline_social_ranking_generation_jobs
  where fixture_provider_id = v_draft.fixture_provider_id
    and scope_provider_id is not distinct from v_draft.scope_provider_id
    and subject_player_provider_id is not distinct from v_draft.subject_player_provider_id
  ...
  if v_job.id is null or v_job.job_state <> 'COMPLETED'
    ...
    raise exception 'TL_SOCIAL_RANKING_APPROVAL_JOB_UNSAFE'; end if;
end
$$;

create or replace function public.touchline_social_044_issue_review_intent(
...
returns jsonb language plpgsql security definer set search_path = '' as $$
declare v_draft public.touchline_social_publication_drafts%rowtype;
  v_job public.touchline_social_ranking_generation_jobs%rowtype; v_id uuid;
  v_lookup_fixture_provider_id text; v_lookup_scope_provider_id text;
  v_lookup_subject_player_provider_id text;
...
begin
  if p_draft_id is null or coalesce(p_review_kind, '') not in ('ARTWORK', 'CAPTION')
    ...
    raise exception 'TL_SOCIAL_RANKING_INTENT_INPUT_INVALID'; end if;
  select fixture_provider_id, scope_provider_id, subject_player_provider_id, content_type, template_version
  into v_lookup_fixture_provider_id, v_lookup_scope_provider_id, v_lookup_subject_player_provider_id,
    v_lookup_content_type, v_lookup_template_version
  from public.touchline_social_publication_drafts
  where id = p_draft_id and content_type in (
    'GAMEWEEK_RANKING_PREVIEW','GAMEWEEK_RANKING_FINAL','PLAYER_DUEL',
    'GAMEWEEK_HERO','TOP_PERFORMER','HAT_TRICK_HERO'
  );
  if v_lookup_fixture_provider_id is null or v_lookup_template_version is null then
    raise exception 'TL_SOCIAL_RANKING_INTENT_STALE'; end if;
  perform pg_catalog.pg_advisory_xact_lock_shared(pg_catalog.hashtextextended('touchline-social-source-revision', 0));
  perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended(
    'touchline-social-044-generation:' || v_lookup_fixture_provider_id || ':'
    || coalesce(v_lookup_scope_provider_id, '0') || ':'
    || coalesce(v_lookup_subject_player_provider_id, '0') || ':' ||
  ...
  if v_draft.fixture_provider_id is distinct from v_lookup_fixture_provider_id
     or v_draft.scope_provider_id is distinct from v_lookup_scope_provider_id
     or v_draft.subject_player_provider_id is distinct from v_lookup_subject_player_provider_id
     ...
     or v_draft.content_type not in (
       'GAMEWEEK_RANKING_PREVIEW','GAMEWEEK_RANKING_FINAL','PLAYER_DUEL',
       'GAMEWEEK_HERO','TOP_PERFORMER','HAT_TRICK_HERO'
    ...
    raise exception 'TL_SOCIAL_RANKING_INTENT_STALE'; end if;
  perform public.touchline_social_044_assert_approval_gate(p_draft_id);
  select * into v_job from public.touchline_social_ranking_generation_jobs
    where generated_draft_id = p_draft_id and job_state = 'COMPLETED' for share;
  insert into public.touchline_social_ranking_review_intents(
...
create or replace function public.touchline_social_044_approve(
...
returns jsonb language plpgsql security definer set search_path = '' as $$
declare v_draft public.touchline_social_publication_drafts%rowtype;
  v_intent public.touchline_social_ranking_review_intents%rowtype;
  v_lookup_fixture_provider_id text; v_lookup_scope_provider_id text;
  v_lookup_subject_player_provider_id text;
  v_lookup_content_type text; v_lookup_template_version text;
begin
  ...
  if p_intent_id is null or p_draft_id is null or coalesce(p_review_kind, '') not in ('ARTWORK', 'CAPTION') then
    raise exception 'TL_SOCIAL_RANKING_APPROVAL_INPUT_INVALID'; end if;
  select fixture_provider_id, scope_provider_id, subject_player_provider_id, content_type, template_version
  into v_lookup_fixture_provider_id, v_lookup_scope_provider_id, v_lookup_subject_player_provider_id,
    v_lookup_content_type, v_lookup_template_version
  from public.touchline_social_publication_drafts
  ...
  if v_lookup_fixture_provider_id is null or v_lookup_template_version is null then
    raise exception 'TL_SOCIAL_RANKING_APPROVAL_STALE'; end if;
  perform pg_catalog.pg_advisory_xact_lock_shared(pg_catalog.hashtextextended('touchline-social-source-revision', 0));
  perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended(
    'touchline-social-044-generation:' || v_lookup_fixture_provider_id || ':'
    || coalesce(v_lookup_scope_provider_id, '0') || ':'
    || coalesce(v_lookup_subject_player_provider_id, '0') || ':' ||
    v_lookup_content_type || ':' || v_lookup_template_version, 0));
  select * into v_intent from public.touchline_social_ranking_review_intents
  ...
  if v_intent.id is null or v_intent.consumed_at is not null or v_intent.expires_at <= clock_timestamp()
    ...
    raise exception 'TL_SOCIAL_RANKING_INTENT_INVALID'; end if;
  select * into v_draft from public.touchline_social_publication_drafts where id = p_draft_id for update;
  if v_draft.fixture_provider_id is distinct from v_lookup_fixture_provider_id
     or v_draft.scope_provider_id is distinct from v_lookup_scope_provider_id
     or v_draft.subject_player_provider_id is distinct from v_lookup_subject_player_provider_id
  ...
  update public.touchline_social_ranking_review_intents set consumed_at = clock_timestamp()
    where id = p_intent_id;
  return jsonb_build_object('ok', true, 'draftId', p_draft_id, 'review', p_review_kind);
end
$$;

-- Preserve the frozen 039-043 approval authorities and route only the six
-- ranking-family types to the independent 044 gate.
...
begin
  if (new.artwork_approval_state = 'APPROVED' and old.artwork_approval_state is distinct from new.artwork_approval_state)
    ...
    elsif new.content_type in ('GOAL_CONFIRMED', 'RED_CARD_CONFIRMED') then
...
create or replace function public.touchline_social_044_enqueue_dispatch(p_draft_id uuid)
returns jsonb language plpgsql security definer set search_path = '' as $$
begin
  raise exception 'TL_SOCIAL_RANKING_DISPATCH_DISABLED';
end
$$;

alter table public.touchline_social_ranking_executor_cycles enable row level security;
alter table public.touchline_social_ranking_executor_cycles force row level security;
alter table public.touchline_social_ranking_generation_jobs enable row level security;
alter table public.touchline_social_ranking_generation_jobs force row level security;
alter table public.touchline_social_ranking_review_intents enable row level security;
alter table public.touchline_social_ranking_review_intents force row level security;

revoke all privileges on table public.touchline_social_ranking_executor_cycles,
  public.touchline_social_ranking_generation_jobs,
  public.touchline_social_ranking_review_intents from public, anon, authenticated, service_role;
grant select on table public.touchline_social_ranking_executor_cycles,
  public.touchline_social_ranking_generation_jobs to service_role;

revoke all on function public.touchline_social_044_claim_cycle(text) from public, anon, authenticated;
revoke all on function public.touchline_social_044_renew_cycle(text, uuid) from public, anon, authenticated;
revoke all on function public.touchline_social_044_complete_cycle(text, uuid, text, text, integer) from public, anon, authenticated;
revoke all on function public.touchline_social_044_create_draft(jsonb) from public, anon, authenticated;
revoke all on function public.touchline_social_044_enqueue_job(uuid, text, text, text, text, text, timestamptz, timestamptz, text, jsonb, text) from public, anon, authenticated;
revoke all on function public.touchline_social_044_claim_job(uuid) from public, anon, authenticated;
revoke all on function public.touchline_social_044_renew_job(uuid, uuid, uuid) from public, anon, authenticated;
revoke all on function public.touchline_social_044_complete_job(uuid, uuid, uuid, text, text, uuid) from public, anon, authenticated;
revoke all on function public.touchline_social_044_assert_approval_gate(uuid) from public, anon, authenticated, service_role;
revoke all on function public.touchline_social_044_issue_review_intent(uuid, text, text, text, text, text, uuid) from public, anon, authenticated, service_role;
revoke all on function public.touchline_social_044_approve(uuid, uuid, text, text, text, text, text, uuid) from public, anon, authenticated;
revoke all on function public.touchline_social_044_enqueue_dispatch(uuid) from public, anon, authenticated, service_role;
grant execute on function public.touchline_social_044_create_draft(jsonb) to service_role;
grant execute on function public.touchline_social_044_claim_cycle(text) to service_role;
grant execute on function public.touchline_social_044_renew_cycle(text, uuid) to service_role;
grant execute on function public.touchline_social_044_complete_cycle(text, uuid, text, text, integer) to service_role;
grant execute on function public.touchline_social_044_enqueue_job(uuid, text, text, text, text, text, timestamptz, timestamptz, text, jsonb, text) to service_role;
grant execute on function public.touchline_social_044_claim_job(uuid) to service_role;
grant execute on function public.touchline_social_044_renew_job(uuid, uuid, uuid) to service_role;
grant execute on function public.touchline_social_044_complete_job(uuid, uuid, uuid, text, text, uuid) to service_role;
grant execute on function public.touchline_social_044_issue_review_intent(uuid, text, text, text, text, text, uuid) to service_role;
grant execute on function public.touchline_social_044_approve(uuid, uuid, text, text, text, text, text, uuid) to authenticated;
