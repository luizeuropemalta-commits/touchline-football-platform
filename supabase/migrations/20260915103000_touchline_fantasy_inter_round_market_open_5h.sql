-- The next Markt window opens five hours after the final provider observation
-- of the preceding Gameweek.  This is forward-only: finalized_at and the
-- fail-closed predecessor guard introduced by the earlier migration remain
-- canonical.
--
-- A five-hour delay can be longer than the interval before the next lock in a
-- compressed calendar.  The table requires market_opens_at < locks_at, so an
-- impossible window is represented by the existing one-microsecond sentinel
-- and remains UPCOMING until the normal locked/live/final state takes over.
-- It must never be opened early merely to make the interval fit.
...
begin
  with round_facts as (
    ...
    group by
      ...
      round.name,
      config.lock_offset_minutes
    having count(fixture.starts_at) > 0
  ), ordered_rounds as (
    select
      round_facts.*,
      row_number() over (
        partition by season_id
        order by round_sort_number nulls last, first_fixture_at, round_id
    ...
    select
      timing.*,
      case
        when round_sequence = 1 then true
        when previous_round_all_final
          and previous_round_completed_at is not null
          and previous_round_completed_at + interval '5 hours' < locks_at then true
        else false
      ...
      case
        ...
        when previous_round_all_final
          and previous_round_completed_at is not null
          and previous_round_completed_at + interval '5 hours' < locks_at
          then previous_round_completed_at + interval '5 hours'
