-- A Card Engine completeness repair must never impersonate a new Market Value
-- review.  Keep the established owner-value command intact for manual value
-- work, and route only the automatic publication source through this strictly
-- publication-only command.
begin;
...
begin
  ...
  select * into v_player from public.football_players where id = p_player_id for update;
  ...
  select * into v_membership from public.football_squad_members as membership
   where membership.id = p_membership_id and membership.player_id = p_player_id
     ...
     and membership.provider = 'sportmonks' and membership.status = 'active'
   for update;
  ...
  if not exists (select 1 from public.football_competitions
    where id = p_competition_id and provider = 'sportmonks' and provider_competition_id = '8') then
  ...
  if not found or v_market_value.status <> 'verified' or v_market_value.confidence <> 'verified'
     or v_market_value.market_value_eur is null
     or v_market_value.market_value_eur <> p_market_value_eur
  ...
  insert into public.touchline_card_publications (
    ...
    v_market_value.id, 'published', p_calculated_tier,
    p_nominal_price_gbp, p_nominal_price_gbp, p_policy_version,
    p_last_reviewed_at, clock_timestamp(), null,
    nullif(trim(coalesce(p_internal_note, '')), ''), p_internal_source,
  ...
  insert into public.touchline_card_publication_history (
    ...
    v_before_state,
    jsonb_build_object('publication', to_jsonb(v_publication), 'market_value', to_jsonb(v_market_value)),
    p_actor_id
  );
...
revoke all on function public.touchline_apply_derived_card_publication(
  uuid, uuid, uuid, text, bigint, text, integer, text, text, timestamptz, text, text, uuid
) from public, anon, authenticated;
grant execute on function public.touchline_apply_derived_card_publication(
  uuid, uuid, uuid, text, bigint, text, integer, text, text, timestamptz, text, text, uuid
) to service_role;

comment on function public.touchline_apply_derived_card_publication(
  uuid, uuid, uuid, text, bigint, text, integer, text, text, timestamptz, text, text, uuid
) is 'Card Engine publication-only command. It reuses a verified value unchanged and never writes football_player_market_values or its history.';

-- Preserve the existing manual Market Value command under an internal name
-- and keep its public RPC signature as a dispatcher.  Existing owner value
-- workflows retain their exact behavior; only the fixed Card Engine source
-- reaches the publication-only command above.
alter function public.touchline_apply_manual_card_publication(
  uuid, uuid, uuid, text, bigint, text, integer, text, text, timestamptz, text, text, uuid
) rename to touchline_apply_manual_card_publication_with_market_value;

create function public.touchline_apply_manual_card_publication(
  p_player_id uuid,
  p_membership_id uuid,
  p_competition_id uuid,
  p_effective_season text,
...
begin
  if p_internal_source = 'touchline_card_engine_auto_publication' then
    return query select * from public.touchline_apply_derived_card_publication(
      p_player_id, p_membership_id, p_competition_id, p_effective_season,
      p_market_value_eur, p_calculated_tier, p_nominal_price_gbp,
      p_policy_version, p_publication_status, p_last_reviewed_at,
      p_internal_note, p_internal_source, p_actor_id
    );
  end if;
...
revoke all on function public.touchline_apply_manual_card_publication_with_market_value(
  uuid, uuid, uuid, text, bigint, text, integer, text, text, timestamptz, text, text, uuid
) from public, anon, authenticated;
grant execute on function public.touchline_apply_manual_card_publication_with_market_value(
  uuid, uuid, uuid, text, bigint, text, integer, text, text, timestamptz, text, text, uuid
) to service_role;
commit;
