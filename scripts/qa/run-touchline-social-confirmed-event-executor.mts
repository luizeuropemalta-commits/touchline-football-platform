...
import {
  discoverTouchlineSocialConfirmedEventCandidates,
  type TouchlineSocialConfirmedEventContentType,
} from "./touchline-social-confirmed-event-candidates.mts";
...
export function assertTouchlineConfirmedEventQaBoundary(environment = process.env) {
  ...
  if (environment.VERCEL_ENV === "production" || projectRef !== QA_PROJECT_REF
    ...
    throw new Error("TL_CONFIRMED_EVENT_EXECUTOR_QA_BOUNDARY_MISMATCH");
  }
...
async function claimCycle(admin: SupabaseClient, component: "SCHEDULER" | "RUNNER") {
  const { data, error } = await admin.rpc("touchline_social_043_claim_cycle", { p_component: component });
  const payload = data as Record<string, unknown> | null;
  const outcome = String(payload?.outcome ?? "");
  if (error) throw new Error(`TL_CONFIRMED_EVENT_${component}_CLAIM_FAILED`);
  if (["busy", "cooldown", "recovered_timeout"].includes(outcome)) return { outcome } as const;
  const leaseToken = String(payload?.leaseToken ?? "");
  if (outcome !== "claimed" || !UUID.test(leaseToken)) throw new Error(`TL_CONFIRMED_EVENT_${component}_CLAIM_INVALID`);
...
async function renewCycle(admin: SupabaseClient, component: "SCHEDULER" | "RUNNER", leaseToken: string) {
  const result = await withDeadline(admin.rpc("touchline_social_043_renew_cycle", {
    p_component: component, p_lease_token: leaseToken,
  }), `TL_CONFIRMED_EVENT_${component}_RENEW_TIMEOUT`);
  if (result.error || (result.data as Record<string, unknown> | null)?.outcome !== "renewed") {
    throw new Error(`TL_CONFIRMED_EVENT_${component}_RENEW_FAILED`);
  }
}
...
async function completeCycle(admin: SupabaseClient, component: "SCHEDULER" | "RUNNER",
  ...
  const { error } = await admin.rpc("touchline_social_043_complete_cycle", {
    p_component: component, p_lease_token: leaseToken, p_outcome: outcome,
    p_error_code: errorCode, p_items_processed: count,
  });
  if (error) throw new Error(`TL_CONFIRMED_EVENT_${component}_COMPLETE_FAILED`);
}
...
function heartbeat(admin: SupabaseClient, component: "SCHEDULER" | "RUNNER", leaseToken: string) {
  ...
  const timer = setInterval(() => {
    ...
    inflight = renewCycle(admin, component, leaseToken)
      .catch((error) => { failure = error instanceof Error ? error : new Error("TL_CONFIRMED_EVENT_RENEW_FAILED"); })
...
export async function runTouchlineConfirmedEventScheduler(input: Readonly<{
  admin: SupabaseClient;
  base: URL;
  renderSecret: string;
  explicitFixtureId?: string | null;
  explicitEventId?: string | null;
  discover?: typeof discoverTouchlineSocialConfirmedEventCandidates;
}>) {
  ...
  let processed = 0;
  try {
    const candidates = await (input.discover ?? discoverTouchlineSocialConfirmedEventCandidates)({
      admin: input.admin, base: input.base, renderSecret: input.renderSecret,
      explicitFixtureId: input.explicitFixtureId, explicitEventId: input.explicitEventId,
    });
    pulse.assert();
    for (const candidate of candidates) {
      await renewCycle(input.admin, "SCHEDULER", claim.leaseToken);
      const { data, error } = await input.admin.rpc("touchline_social_043_enqueue_job", {
        p_scheduler_lease_token: claim.leaseToken,
        p_fixture_provider_id: candidate.fixtureId,
        p_event_provider_id: candidate.eventId,
      ...
      if (error || !UUID.test(String((data as Record<string, unknown> | null)?.jobId ?? ""))) {
        throw new Error("TL_CONFIRMED_EVENT_SCHEDULER_ENQUEUE_FAILED");
      }
      processed += 1;
    }
    await pulse.stop();
    ...
    const code = safeCode(error, "TL_CONFIRMED_EVENT_SCHEDULER_FAILED");
...
async function claimJob(admin: SupabaseClient, runnerLeaseToken: string): Promise<ClaimedJob | null> {
  const { data, error } = await admin.rpc("touchline_social_043_claim_job", { p_runner_lease_token: runnerLeaseToken });
  const payload = data as Record<string, unknown> | null;
  if (error) throw new Error("TL_CONFIRMED_EVENT_RUNNER_JOB_CLAIM_FAILED");
  if (payload?.outcome === "empty") return null;
  const job = {
    jobId: String(payload?.jobId ?? ""), leaseToken: String(payload?.leaseToken ?? ""),
    fixtureId: String(payload?.fixtureId ?? ""), eventId: String(payload?.eventId ?? ""),
    contentType: String(payload?.contentType ?? "") as TouchlineSocialConfirmedEventContentType,
...
async function renewJob(admin: SupabaseClient, runnerLeaseToken: string, job: ClaimedJob) {
  const result = await withDeadline(admin.rpc("touchline_social_043_renew_job", {
    p_runner_lease_token: runnerLeaseToken, p_job_id: job.jobId, p_job_lease_token: job.leaseToken,
  }), "TL_CONFIRMED_EVENT_RUNNER_JOB_RENEW_TIMEOUT");
  if (result.error || (result.data as Record<string, unknown> | null)?.outcome !== "renewed") {
    throw new Error("TL_CONFIRMED_EVENT_RUNNER_JOB_RENEW_FAILED");
  }
}
...
async function completeJob(admin: SupabaseClient, runnerLeaseToken: string, job: ClaimedJob,
  ...
  const { error } = await admin.rpc("touchline_social_043_complete_job", {
  ...
  if (error) throw new Error("TL_CONFIRMED_EVENT_RUNNER_JOB_COMPLETE_FAILED");
}

async function generate(job: ClaimedJob) {
  const generator = fileURLToPath(new URL("./generate-touchline-social-confirmed-event-draft.mts", import.meta.url));
  const child = spawn(process.execPath, ["--experimental-strip-types", generator,
    `--fixture-id=${job.fixtureId}`, `--event-id=${job.eventId}`,
  ...
  if (timedOut) throw new Error("TL_CONFIRMED_EVENT_GENERATOR_TIMEOUT");
  if (code !== 0) throw new Error("TL_CONFIRMED_EVENT_GENERATOR_FAILED");
  ...
  if (payload.outcome !== "generated" || !UUID.test(draftId)
    ...
    throw new Error("TL_CONFIRMED_EVENT_GENERATOR_RESULT_INVALID");
  }
  return draftId;
}

export async function runTouchlineConfirmedEventRunner(input: Readonly<{
  admin: SupabaseClient;
  ...
  try {
    ...
    const code = safeCode(error, "TL_CONFIRMED_EVENT_RUNNER_FAILED");
    await pulse?.stop().catch(() => undefined);
...
async function main() {
  ...
  if (!["scheduler", "runner"].includes(mode ?? "")) throw new Error("TL_CONFIRMED_EVENT_EXECUTOR_MODE_INVALID");
  const boundary = assertTouchlineConfirmedEventQaBoundary();
  const admin = createClient(boundary.supabaseUrl, boundary.serviceRole, {
    auth: { autoRefreshToken: false, persistSession: false },
  });
  const result = mode === "scheduler"
    ? await runTouchlineConfirmedEventScheduler({ admin, base: boundary.base,
      renderSecret: boundary.renderSecret, explicitFixtureId: argument("fixture-id"),
      explicitEventId: argument("event-id") })
    : await runTouchlineConfirmedEventRunner({ admin });
  process.stdout.write(`${JSON.stringify({ service: `touchline-confirmed-event-${mode}`,
    projectRef: boundary.projectRef, host: boundary.base.hostname, ...result })}\n`);
}

if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  main().catch((error) => {
    process.stderr.write(`${JSON.stringify({ service: "touchline-confirmed-event-043",
      outcome: "failure", code: safeCode(error, "TL_CONFIRMED_EVENT_EXECUTOR_FATAL") })}\n`);
    process.exitCode = 1;
  });
}
