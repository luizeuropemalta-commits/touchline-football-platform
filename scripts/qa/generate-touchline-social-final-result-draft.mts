...
const CONFIG = Object.freeze({
  FULL_TIME: { templateVersion: "touchline-full-time-feed-v1", placement: "INSTAGRAM_FEED", height: 1350 },
  FINAL_SCORE: { templateVersion: "touchline-final-score-story-v1", placement: "INSTAGRAM_STORY", height: 1920 },
});

function required(name: string) {
  const value = process.env[name]?.trim();
  if (!value) throw new Error(`TL_FINAL_RESULT_GENERATOR_ENV_MISSING:${name}`);
  return value;
}
...
const contentType = argument("content-type") as "FULL_TIME" | "FINAL_SCORE";
...
if (!/^[1-9]\d{0,19}$/.test(fixtureId) || !config || !SHA256.test(expectedInputChecksum)
  || !SHA256.test(expectedSourceRevisionChecksum)) throw new Error("TL_FINAL_RESULT_GENERATOR_ARGS_INVALID");
...
if (process.env.VERCEL_ENV === "production" || projectRef !== QA_PROJECT_REF
  ...
  throw new Error("TL_FINAL_RESULT_GENERATOR_QA_BOUNDARY_MISMATCH");
}
...
try {
  const context = await browser.newContext({ viewport: { width: 1080, height: config.height }, deviceScaleFactor: 1 });
  ...
  try {
    const [existing, queuedJob] = await Promise.all([
      admin.from("touchline_social_publication_drafts").select("revision")
        ...
        .eq("placement", config.placement).eq("locale", "en-GB")
        .eq("template_version", config.templateVersion)
        .order("revision", { ascending: false }).limit(1).maybeSingle(),
      admin.from("touchline_social_final_result_generation_jobs")
    ...
    if (existing.error || queuedJob.error || queuedJob.data?.job_state !== "RUNNING"
      ...
      throw new Error("TL_FINAL_RESULT_GENERATION_JOB_IDENTITY_INVALID");
    }
    const revision = Number(existing.data?.revision ?? 0) + 1;
    const renderPath = touchlineSocialRenderPath({ fixtureId, teamId: null,
      contentType: contentType as TouchlineSocialContentType, locale: "en-GB", revision });
    const response = await page.goto(new URL(renderPath, base).toString(), { waitUntil: "domcontentloaded", timeout: 45_000 });
    if (!response?.ok()) throw new Error(`TL_FINAL_RESULT_RENDER_HTTP_${response?.status() ?? 0}`);
    await Promise.race([page.evaluate(() => document.fonts.ready),
      new Promise((_, reject) => setTimeout(() => reject(new Error("TL_FINAL_RESULT_FONT_TIMEOUT")), 15_000))]);
    const canvas = page.locator('[data-social-art="touchline-final-score"]');
    if (await canvas.count() !== 1) throw new Error("TL_FINAL_RESULT_RENDER_NOT_READY");
    const metadata = await canvas.evaluate((node) => {
      ...
      return {
        ...
        placement: element.dataset.socialPlacement,
        ...
        scoreState: element.dataset.scoreState,
        width: box.width,
        height: box.height,
    ...
    if (metadata.contentType !== contentType || metadata.placement !== (contentType === "FULL_TIME" ? "feed" : "story")
      ...
      || metadata.scoreState !== "finished" || metadata.width !== 1080 || metadata.height !== config.height
      || !metadata.imagesReady || metadata.overflow
      || /\b(?:sportmonks|api|provider|pipeline|settlement|fixture\s+\d+)\b/i.test(metadata.publicText ?? "")) {
      throw new Error("TL_FINAL_RESULT_RENDER_CONTRACT_MISMATCH");
    }
    ...
    const placement = config.placement as TouchlineSocialPlacement;
    const objectKey = touchlineSocialArtifactObjectKey({ fixtureId, teamId: null,
      contentType: contentType as TouchlineSocialContentType, placement, locale: "en-GB", revision,
    ...
    if (!sourceResponse.ok || current?.sourceChecksum !== expectedInputChecksum
      ...
      throw new Error("TL_FINAL_RESULT_SOURCE_CHANGED_DURING_RENDER");
    }
    const sourceSnapshotAt = new Date(metadata.sourceSnapshotAt!).toISOString();
    const generatedAt = new Date().toISOString();
    const draftResult = await createTouchlineSocialPublicationDraft({
      fixtureId, teamId: null, contentType: contentType as TouchlineSocialContentType,
      placement, locale: "en-GB", revision, renderPath, caption: metadata.caption,
      firstObservedAt, sourceSnapshotAt, templateVersion: config.templateVersion,
    ...
    if (!draftResult.ok) throw new Error(`TL_FINAL_RESULT_DRAFT_CONTRACT_${draftResult.reason}`);
    const draft = draftResult.draft;
    const inserted = await admin.rpc("touchline_social_create_draft", { p_draft: {
      publication_key: draft.publicationKey, fixture_provider_id: draft.fixtureId, team_provider_id: null,
      content_type: draft.contentType, placement: draft.placement, locale: draft.locale,
      revision: draft.revision, render_path: draft.renderPath, width: draft.width, height: draft.height,
      caption: draft.caption, first_observed_at: draft.firstObservedAt,
      source_snapshot_at: draft.sourceSnapshotAt, generated_at: draft.generatedAt,
      template_version: draft.templateVersion, source_version: draft.sourceVersion,
      source_checksum: draft.sourceChecksum, source_revision_manifest: draft.sourceRevisionManifest,
    ...
    if (inserted.error || !UUID.test(draftId)) throw new Error("TL_FINAL_RESULT_DRAFT_INSERT_FAILED");
    process.stdout.write(`${JSON.stringify({ outcome: "generated", contentType, fixtureId, draftId,
