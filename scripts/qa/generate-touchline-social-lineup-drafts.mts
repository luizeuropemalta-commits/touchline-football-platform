...
function reviewReason(value: string) {
  const normalized = value.split(":", 1)[0]!.toUpperCase().replace(/[^A-Z0-9_:-]+/g, "_").slice(0, 160);
...
function parseSourceRevision(value: unknown) {
  ...
  if (!manifestValue || typeof manifestValue !== "object" || Array.isArray(manifestValue)
    || !/^sha256:[a-f0-9]{64}$/.test(checksum)) return null;
...
async function readMinimalSourceRevision(fixtureId: string) {
  const { data, error } = await admin.rpc("touchline_social_read_source_revision", {
    p_source_keys: [`fixture-provider:${fixtureId}`],
  });
  if (error) throw new Error("TL_SOCIAL_GENERATOR_SOURCE_REVISION_UNAVAILABLE");
  const parsed = parseSourceRevision(data);
  if (!parsed) throw new Error("TL_SOCIAL_GENERATOR_SOURCE_REVISION_INVALID");
  return parsed;
}

async function readCurrentSource(candidate: Candidate) {
  const url = new URL("/api/admin/social-publications/source", base);
  url.searchParams.set("fixtureId", candidate.fixtureId);
  url.searchParams.set("teamId", candidate.teamId);
  const response = await fetch(url, {
    method: "GET",
    headers: { cookie: `tl-social-render=${encodeURIComponent(renderSecret)}` },
    cache: "no-store",
  ...
  if (!response.ok
    || payload?.ok !== true
    || String(payload.fixtureId ?? "") !== candidate.fixtureId
    || String(payload.teamId ?? "") !== candidate.teamId
    || !String(payload.sourceVersion ?? "").match(/^[A-Za-z0-9._-]{1,160}$/)
    || !String(payload.sourceChecksum ?? "").match(/^sha256:[a-f0-9]{64}$/)
    || !Number.isFinite(Date.parse(String(payload.sourceSnapshotAt ?? "")))) {
    throw new Error("TL_SOCIAL_GENERATOR_CURRENT_SOURCE_UNAVAILABLE");
  }
  const sourceRevision = parseSourceRevision(payload);
  if (!sourceRevision) throw new Error("TL_SOCIAL_GENERATOR_SOURCE_REVISION_INVALID");
  return {
...
async function discoverCandidates(): Promise<Candidate[]> {
  const explicitFixtureId = argument("fixture-id");
  const explicitTeamId = argument("team-id");
  if (explicitFixtureId || explicitTeamId) {
    if (!explicitFixtureId || !explicitTeamId || !NUMERIC_ID.test(explicitFixtureId) || !NUMERIC_ID.test(explicitTeamId)) {
      throw new Error("TL_SOCIAL_GENERATOR_EXPLICIT_IDENTITY_INVALID");
    }
  }
  ...
  if (competitionResult.error || !competitionId) throw new Error("TL_SOCIAL_GENERATOR_COMPETITION_READ_FAILED");
  ...
  if (explicitFixtureId) {
    const fixtureResult = await admin
      ...
      .maybeSingle();
    if (fixtureResult.error) throw new Error("TL_SOCIAL_GENERATOR_FIXTURE_READ_FAILED");
    if (!fixtureResult.data) throw new Error("TL_SOCIAL_GENERATOR_EXPLICIT_FIXTURE_NOT_FOUND");
    ...
    if (lifecycleResult.error || !Array.isArray(lifecycleResult.data)) {
      throw new Error("TL_SOCIAL_GENERATOR_LIFECYCLE_READ_FAILED");
    }
    if (!lifecycleResult.data.length) throw new Error("TL_SOCIAL_GENERATOR_EXPLICIT_LINEUP_NOT_OBSERVED");
    ...
    const fixturesResult = await admin
      .from("football_fixtures")
      .select(fixtureSelect)
      .in("id", fixtureIds)
      .eq("provider", "sportmonks")
    ...
    if (fixturesResult.error || !Array.isArray(fixturesResult.data)) {
      throw new Error("TL_SOCIAL_GENERATOR_FIXTURE_READ_FAILED");
    }
    fixtures = fixturesResult.data;
  }
  const clubIds = [...new Set(fixtures.flatMap((fixture) => [fixture.home_club_id, fixture.away_club_id]).filter(Boolean))];
  const { data: clubs, error: clubsError } = clubIds.length
    ? await admin.from("football_clubs").select("id,provider,provider_team_id").in("id", clubIds).eq("provider", "sportmonks")
    : { data: [], error: null };
  if (clubsError || !Array.isArray(clubs)) throw new Error("TL_SOCIAL_GENERATOR_CLUB_READ_FAILED");
  const providerFixtureIds = fixtures.map((fixture) => String(fixture.provider_fixture_id ?? "")).filter((id) => NUMERIC_ID.test(id));
  ...
  if (feedsResult.error || !Array.isArray(feedsResult.data)) {
    throw new Error("TL_SOCIAL_GENERATOR_FEED_PROBE_FAILED");
  }
  ...
  const teamByClubId = new Map(clubs.map((club) => [String(club.id), String(club.provider_team_id)]));
  const firstObservedByFixtureId = new Map(lifecycle.map((row) => [String(row.fixture_id), String(row.first_observed_at)]));
  const now = Date.now();
  const candidates = fixtures.flatMap((fixture) => {
    const fixtureId = String(fixture.provider_fixture_id ?? "");
    const startsAt = Date.parse(String(fixture.starts_at ?? ""));
    const startsAtIso = String(fixture.starts_at ?? "");
    const firstObservedAt = firstObservedByFixtureId.get(String(fixture.id)) ?? "";
    // This fingerprint only controls retry/cooldown before the typed source
    // endpoint is ready. Volatile persistence timestamps must not create a
    // fresh REVIEW_REQUIRED attempt for semantically identical football data.
    const inputChecksum = discoveryFingerprintByFixtureId.get(fixtureId) ?? "";
    const teamIds = [teamByClubId.get(String(fixture.home_club_id)), teamByClubId.get(String(fixture.away_club_id))];
    if (!NUMERIC_ID.test(fixtureId)
      || !Number.isFinite(startsAt)
      || (!explicitFixtureId && startsAt < now - 24 * 60 * 60 * 1000)
      || (!explicitFixtureId && startsAt > now + 36 * 60 * 60 * 1000)
  ...
  return Promise.all(candidates.map(async (candidate) => {
    try {
      const current = await readCurrentSource(candidate);
      return {
        ...candidate,
      ...
      // Source incompleteness is a per-candidate review outcome. It must not
      // abort discovery for the other club/fixture candidates in this cycle.
      const minimalRevision = await readMinimalSourceRevision(candidate.fixtureId);
      return {
        ...candidate,
        ...minimalRevision,
        sourceReadiness: "REVIEW_REQUIRED" as const,
        sourceReasonCode: reviewReason(error instanceof Error ? error.message : "SOURCE_NOT_READY"),
      };
    }
  }));
}
