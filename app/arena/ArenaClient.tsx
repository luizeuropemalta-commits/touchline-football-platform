...
function arenaPlayerZoomDetails(
  ...
  return buildTouchlinePlayerCardZoomDetails({
    ...
    extraFields: [
      ...
      { label: locale === "pt-BR" ? "Nota da última partida" : "Last match rating", value: player.matchRating == null ? "—" : String(player.matchRating), accent: true, kind: "rating-last", icon: "rating" },
      { label: locale === "pt-BR" ? "Gols na temporada" : "Season goals", value: player.seasonStats?.goals == null ? "—" : String(player.seasonStats.goals), icon: "goal" },
      { label: locale === "pt-BR" ? "Assistências na temporada" : "Season assists", value: player.seasonStats?.assists == null ? "—" : String(player.seasonStats.assists), icon: "assist" },
      { label: locale === "pt-BR" ? "Cartões na temporada" : "Season cards", value: player.seasonStats?.cards == null ? "—" : String(player.seasonStats.cards), icon: "cards" },
      ...buildTouchlineVerifiedMatchFactFields({
    ...
    >
      ...
      <style>{`
        ...
        /* Twenty-two live cards deliberately stay on static paint layers.
           Their card perimeter trace is disabled here so Safari never
           composites a new animated layer for every player. */
        .arena-live-moving-card .arena-live-compact-card-product > .touchline-card-surface [data-touchline-card-neon-trace-run="true"],
        .arena-live-coach-card .arena-live-coach-card-art.touchline-card-surface [data-touchline-card-neon-trace-run="true"] {
