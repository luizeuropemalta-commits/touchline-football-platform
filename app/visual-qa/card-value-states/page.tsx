...
function StaticCanonicalCard({ player }: { player: TouchlineEliteExactPlayer }) {
  return (
...
export default function CardValueStatesVisualQaPage() {
  return (
    <main
    ...
    >
      <header style={{ width: "min(1280px, 100%)", margin: "0 auto", borderBottom: "1px solid rgba(186, 230, 253, .22)", paddingBottom: 22 }}>
        ...
        <h1 style={{ margin: "9px 0 0", fontSize: "clamp(31px, 5vw, 56px)", letterSpacing: "-.05em", lineHeight: 1 }}>Canonical card value states</h1>
        <p style={{ maxWidth: 800, margin: "15px 0 0", color: "rgba(226,232,240,.72)", fontSize: 15, lineHeight: 1.65 }}>
          Three synthetic fixtures exercise the server-owned public presentation boundary. They contain no player, roster, provider, contract or account data.
        </p>
      </header>
      ...
      >
        {CARD_FIXTURES.map((fixture) => (
          ...
          >
            <div>
              <p style={{ margin: 0, color: "#bae6fd", fontSize: 11, fontWeight: 900, letterSpacing: ".1em" }}>{fixture.label}</p>
              <p style={{ minHeight: 50, margin: "9px 0 0", color: "rgba(226,232,240,.70)", fontSize: 14, lineHeight: 1.55 }}>{fixture.description}</p>
            </div>
            <StaticCanonicalCard player={fixture.player} />
            <dl style={{ display: "grid", gap: 8, margin: 0, color: "rgba(226,232,240,.78)", fontSize: 13 }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 14 }}><dt style={{ color: "rgba(186,230,253,.62)" }}>Expected border</dt><dd style={{ margin: 0, fontWeight: 850, textAlign: "right" }}>{fixture.expectedTier}</dd></div>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 14 }}><dt style={{ color: "rgba(186,230,253,.62)" }}>Expected presentation</dt><dd style={{ margin: 0, fontWeight: 850, textAlign: "right" }}>{fixture.expectedCommercialState}</dd></div>
            </dl>
          </article>
        ))}
      </section>
      ...
      >
        Static visual fixture only. Ranking subscription, card actions, profile links, neon selection, layout editing and persisted layout access are disabled.
      </aside>
    </main>
  );
}
