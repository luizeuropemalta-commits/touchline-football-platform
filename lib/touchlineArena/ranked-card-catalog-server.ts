...
/** Public published-card catalogue decorated from one coherent active score version. */
export async function loadTouchLineRankedCardCatalog(
  ...
  if (state.phase !== "ranked" || (state.scoringVersion !== "player_scoring_v2" && state.scoringVersion !== "player_scoring_v3") || !state.seasonId || !state.players.length) return [];
  const admin = providedAdmin ?? createAdminClient();
  if (!admin) return [];
  const scoringVersion = state.scoringVersion;
  const settlementTable = scoringVersion === "player_scoring_v3"
    ? "touchline_player_fixture_score_settlements"
    : "football_player_fixture_statistics";
  ...
  const [{ data: playerData, error: playerError }, { data: squadData, error: squadError }, { data: seasonData, error: seasonError }, { data: fixtureData, error: fixtureError }] = await Promise.all([
    ...
    admin.from("football_player_season_statistics").select("football_player_id,summary_payload,position_statistics_payload").eq("season_id", state.seasonId).eq("scoring_version", scoringVersion).in("football_player_id", playerIds),
    admin.from(settlementTable).select("football_player_id,touchline_points,touchline_points_breakdown,rating,statistics_payload,football_fixtures!inner(starts_at)").eq("season_id", state.seasonId).eq("scoring_version", scoringVersion).in("football_player_id", playerIds).order("starts_at", { referencedTable: "football_fixtures", ascending: false }),
  ]);
