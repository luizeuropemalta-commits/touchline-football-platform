...
const SOURCE_REVISION_KEY = /^(fixture-provider|fixture|competition|season|round|club|player|formation|coach-ranking|card-ranking):[A-Za-z0-9._-]{1,160}$/;
const ARTIFACT_BUCKET = "touchline-social-drafts";
const SOCIAL_DESTINATION = "TOUCHLINE_OFFICIAL_INSTAGRAM";

export type TouchlineSocialContentType = "LINEUP" | "FINAL_SCORE";
 ...
 * Keep the field order byte-for-byte aligned with
 * public.touchline_social_enqueue_dispatch in QA migration 039.
 */
...
export function touchlineSocialRenderPath(input: Readonly<{
  ...
  const base = input.contentType === "LINEUP"
...
export async function createTouchlineSocialPublicationDraft(input: Readonly<{
  ...
  if (input.contentType === "FINAL_SCORE" && teamId) {
    return { ok: false, reason: "FINAL_SCORE_TEAM_ID_FORBIDDEN" } as const;
  }
