...
| 2 | Unified Card Engine and Market Transfer journey | CODE_COMPLETE | `77ffadf8`, `67b8c727`, `18134f7f` and `f347f2dc` preserve stored active-contract tier authority through Arena, ClubOwner, exact cards and ClubHub. Coach acquisition belongs exclusively to Market Transfer: coach → goalkeeper → defence → midfield → attack. A newly registered ClubOwner now receives a 6.5-second localized Arena welcome before continuing to Market Transfer; normal Arena visits are never redirected. `e0e555e9` also prevents any saved player or coach card from mounting on an idle Arena while account state is reconciled. | Validate the welcome, coach persistence and guided sequence with a controlled non-financial ClubOwner persona. |
...
| 9 | Preview validation | IN_PROGRESS | Preview `dpl_Bxj9rrsGrbnxgzK7yVgmMJNXPKL3` for application checkpoint `e0e555e9` is Ready at `touchline-arena-official-cyj6gza71-fifa-agent-plataform.vercel.app`. Local temporal validation confirmed zero player cards, zero coach cards and zero owner field layers on the idle Arena after 3s, 10s, reload, desktop and mobile. Explicit demo matchday still renders 11 players and one coach. Public unauthenticated Preview correctly redirects to localized first-party login. No production alias changed. | Run the controlled authenticated ClubOwner journey and device/browser matrix on this Preview. |
...
`safety/touchline-2026-06-28-wip`; application checkpoint `e0e555e9` contains
the validated idle-Arena correction and remains the active Preview baseline.
