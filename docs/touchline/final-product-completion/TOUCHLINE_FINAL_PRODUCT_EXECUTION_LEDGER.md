...
| 2 | Unified Card Engine and Market Transfer journey | PRODUCTION_VALIDATED | `0c8e3f99` publishes the mandatory coach → formation → 3 GK → 6 CB → 2 RB → 2 LB → 3 CDM → 6 MID → 8 ATT → 5 ST sequence. The limits total exactly 35. Future steps stay locked during first build; full positions enter the existing no-refund replacement flow. Production DOM, desktop and 390×844 mobile were validated without horizontal page overflow. Deployment `dpl_12WhAtuYc5x8VpEU7XmQtXwZpFfG` is Ready on `touchline.com.br`. | Validate one real funded purchase/replacement journey with a controlled ClubOwner; do not alter pricing or activate real payments. |
