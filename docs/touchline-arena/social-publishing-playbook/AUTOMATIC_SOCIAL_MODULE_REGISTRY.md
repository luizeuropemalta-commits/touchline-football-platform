...
| 043 | `GOAL_CONFIRMED`, `RED_CARD_CONFIRMED` | Current fixture plus confirmed, non-pending event revision; goal rejects VAR/pending and red card requires canonical dismissal state | Story 1080×1920 |
| 044 | `GAMEWEEK_RANKING_PREVIEW`, `GAMEWEEK_RANKING_FINAL`, `PLAYER_DUEL`, `GAMEWEEK_HERO`, `TOP_PERFORMER`, `HAT_TRICK_HERO` | One current audited ranking/settlement revision; `FINAL` only after every scoreable fixture and settlement is final | Feed 1080×1350, with separately versioned Story only when specified |
