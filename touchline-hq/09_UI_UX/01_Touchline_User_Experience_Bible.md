...
```mermaid
flowchart TD
  A["Open Touchline"] --> B["Welcome Screen"]
  B --> C["Create Account"]
  C --> D["Create Club"]
  D --> E["Upload Owner Photo"]
  E --> F["AI Generates Club Owner Avatar"]
  F --> G["Choose Club Name"]
  G --> H["Choose Club Badge"]
  H --> I["Receive Starter Budget"]
  I --> J["Guided Tutorial"]
  J --> K["Transfer Center"]
  K --> L["Buy First Players"]
  L --> M["Create Starting XI"]
  M --> N["Choose Coach"]
  N --> O["Enter First Competition"]
  O --> P["First Match"]
  P --> Q["Live Arena"]
  Q --> R["First Ranking"]
  R --> S["First Trophy or First Lesson"]
  S --> T["Promotion Path"]
  T --> U["Long-Term Club Progression"]
```
...
sequenceDiagram
  ...
  participant T as Touchline
  participant M as Market
  participant C as Competition
  participant L as Live Arena

  U->>T: Opens Touchline
  T->>U: Welcome and club creation
  U->>T: Creates club and owner profile
  T->>U: Generates Owner Card
  T->>U: Gives starter budget
  U->>M: Buys first players
  U->>T: Creates Starting XI
  U->>C: Enters first competition
  C->>L: Matchday begins
  L->>U: Live events and ranking
  T->>U: Rewards, progression and next objective
```
