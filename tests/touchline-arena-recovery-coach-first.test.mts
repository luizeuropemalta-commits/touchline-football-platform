...
test("Market Transfer keeps production coach identity out of the demo fallback and owns persistent coach-first selection", () => {
  ...
  assert.match(
    arena,
    /const shouldRenderArenaOwnerLayer = shouldRenderPlayers[\s\S]*?&& \(isArenaMatchdayViewActive \|\| isQuickSubstitutionSessionActive \|\| isDemoLineup\)[\s\S]*?&& standaloneExperience !== "live"[\s\S]*?&& !isCoachSelectionRequired/,
  );
