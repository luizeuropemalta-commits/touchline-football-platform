...
test("reader and template are persisted-only, revision-fenced and contain no XI data surface", () => {
  ...
  assert.match(component, /ensureStaticNameFit/);
  assert.match(component, /TOUCHLINE_SOCIAL_ARENA_GLASS_CSS_VARIABLES/);
  assert.match(componentStyles, /var\(--touchline-social-glass-outer-start\)/);
  assert.match(componentStyles, /var\(--touchline-social-glass-inner-end\)/);
  assert.doesNotMatch(componentStyles, /\.duel\s*\{[^}]*background:\s*(?:#000|black)/s);
