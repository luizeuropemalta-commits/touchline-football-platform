...
type Override = Readonly<{ id: string; field_key: string; status: string; player_id: string; effective_value: unknown; source_batch_id: string | null; approved_at: string | null }>;
...
export function AdminCardEngineConsole({ players, batches, overrides, migrationReady }: { players: readonly Player[]; batches: readonly Batch[]; overrides: readonly Override[]; migrationReady: boolean }) {
  ...
  return <div className="space-y-6">
    ...
    {tab === "inbox" ? <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{players.slice(0, 10).map((player) => <article key={player.id} className="rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-sm font-black text-white">{player.name}</p><p className="mt-1 text-xs text-slate-500">{player.club} · Sportmonks {player.providerPlayerId}</p><p className="mt-3 text-[10px] font-black text-amber-200">PENDING CARD REVIEW</p><button onClick={() => addSingle(player.providerPlayerId)} className="mt-3 rounded-lg border border-cyan-300/30 px-3 py-2 text-[10px] font-black text-cyan-100">Open safely</button></article>)}</section> : null}
    {tab === "single" ? <section className="rounded-2xl border border-white/10 bg-black/20 p-5"><label className="text-xs font-black text-white">Canonical provider player <span className="text-slate-500">(provider identity is blocked from editing)</span></label><select onChange={(event) => addSingle(event.target.value)} className="mt-3 w-full rounded-xl border border-white/10 bg-slate-950 px-3 py-3 text-sm text-white"><option value="">Choose canonical player</option>{options}</select><p className="mt-4 text-xs text-slate-500">Edit only display name, shirt number, approved manual Market Value, or card template. Football facts and transfers stay in Sportmonks.</p></section> : null}
    ...
    {tab === "overrides" ? <section className="space-y-2">{overrides.map((override) => <article key={override.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/10 bg-black/20 p-4"><div><p className="text-sm font-black text-white">{override.field_key}</p><p className="mt-1 font-mono text-[10px] text-slate-500">{override.player_id}</p></div><div className="text-right"><p className="text-xs font-bold text-cyan-100">effective {format(override.effective_value)}</p><p className="mt-1 text-[10px] font-black text-slate-500">{override.status}</p></div></article>)}</section> : null}
    {tab === "quality" ? <section className="rounded-2xl border border-white/10 bg-black/20 p-5"><h2 className="font-black text-white">Data Quality gates</h2><ul className="mt-3 space-y-2 text-sm text-slate-400"><li>• Provider ID and football facts are never editable.</li><li>• Name + club rows are manual-review-only; conflicts/unmatched rows cannot publish.</li><li>• Every override retains provider value, TouchLine override, effective value and stale signal.</li><li>• Missing image / shirt number remain visible as a quality alert; they do not fabricate data.</li></ul></section> : null}
