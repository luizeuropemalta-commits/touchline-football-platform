...
export function CardEngineInbox({ rows, locale }: { rows: readonly CardEngineInboxRow[]; locale: "en-GB" | "pt-BR" }) {
  const [filter, setFilter] = useState<"all" | TouchlineCardReviewField>("all");
  const [openPlayerId, setOpenPlayerId] = useState(rows[0]?.playerId ?? "");
  const [busy, setBusy] = useState(false);
  const [status, setStatus] = useState("");
  const visibleRows = useMemo(() => rows.filter((row) => filter === "all" || row.cardReview.missingFields.includes(filter)), [rows, filter]);
  ...
  return <section className="grid gap-5 rounded-3xl border border-slate-300/20 bg-slate-950/70 p-5 shadow-2xl shadow-black/20 lg:grid-cols-[minmax(0,1fr)_minmax(340px,.8fr)]">
    <div>
      ...
      <div className="mt-4 flex flex-wrap gap-2">{(["all", ...Object.keys(counters)] as Array<"all" | TouchlineCardReviewField>).map((key) => <button key={key} type="button" onClick={() => setFilter(key)} className={`rounded-full border px-3 py-1.5 text-[10px] font-black ${filter === key ? "border-slate-100 bg-slate-100 text-slate-950" : "border-white/10 bg-white/[.03] text-slate-300"}`}>{key === "all" ? `${pt ? "TOTAL REVIEW REQUIRED" : "TOTAL REVIEW REQUIRED"}: ${rows.length}` : `${touchlineCardReviewFieldLabel(key, locale)}: ${counters[key]}`}</button>)}</div>
