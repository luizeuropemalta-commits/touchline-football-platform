...
export default function TouchlineGameweekTeamSnapshot({ snapshot, locale, surface }: {
  ...
  return <section className={styles.shell} data-gameweek-team-surface={surface}>
    ...
    <footer><div><CalendarClock /><span>{gameweek ? new Intl.DateTimeFormat(locale, { dateStyle: "medium", timeStyle: "short" }).format(new Date(gameweek.locksAt)) : "—"}</span>{editable ? <em>{pt ? "EDIÇÃO ABERTA" : "EDITING OPEN"}</em> : <em><LockKeyhole />{pt ? "SNAPSHOT BLOQUEADO" : "LOCKED SNAPSHOT"}</em>}</div><a href={`/market-transfer?lang=${encodeURIComponent(locale)}`}>{editable ? <Pencil /> : <LockKeyhole />}{editable ? (pt ? "Editar no Markt" : "Edit in Markt") : (pt ? "Ver no Markt" : "View in Markt")}</a></footer>
  </section>;
}
