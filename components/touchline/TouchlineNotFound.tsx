...
export default function TouchlineNotFound() {
  ...
  const copy = pt ? {
    ...
    arena: "Voltar para a Arena",
    clubHub: "Explorar ClubHub",
    code: "Erro 404",
  } : {
  ...
  return (
    <main className="min-h-dvh px-5 py-[max(2rem,env(safe-area-inset-top))] text-[#edf8ff]">
      <section className="mx-auto grid min-h-[calc(100dvh-4rem)] max-w-3xl place-items-center">
        <div className="w-full overflow-hidden rounded-[2rem] border border-cyan-200/20 bg-[#07131a]/90 p-7 shadow-[0_24px_80px_rgba(0,0,0,.45)] backdrop-blur-xl sm:p-11">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <Link href={touchlineArenaHref(locale)} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#a3ff12] px-5 text-sm font-black text-[#07110b] transition hover:bg-[#c1ff64]">{copy.arena}<ArrowRight aria-hidden="true" size={17} /></Link>
            <Link href={touchlineClubHubHref(locale)} className="inline-flex min-h-12 items-center justify-center rounded-xl border border-cyan-200/20 bg-white/[.03] px-5 text-sm font-bold text-cyan-100 transition hover:border-cyan-200/45 hover:bg-cyan-200/[.06]">{copy.clubHub}</Link>
          </div>
        </div>
      </section>
    </main>
