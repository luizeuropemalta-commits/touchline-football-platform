...
const portugueseTitles: Record<string, string> = {
  ...
  "club-owner/hub": "Club Hub", "club-owner/training": "Centro de treino", "club-owner/history": "Histórico", "club-owner/renewals": "Renovações", "club-owner/substitutions": "Substituições", "club-owner/frozen": "Clube congelado",
...
function AuditClubOwnerPage({ route, language, player, coachSlot }: { route: TouchlineAuditRoute; language: string; player: TouchlineEliteExactPlayer; coachSlot: ReturnType<typeof createTouchlineArenaCoachSlot> }) {
  ...
  const pt = language === "pt-BR";
  const navigation = <header className={styles.productNav}><strong>ClubHub</strong><span>{pt ? "Centro de treino" : "Training centre"}</span><span>{pt ? "Histórico" : "History"}</span><span>{pt ? "Renovações" : "Renewals"}</span><span>{pt ? "Substituições" : "Substitutions"}</span></header>;
  ...
  if (route.id === "club-owner/training") return <section className={styles.realSurface}>{navigation}<div className={styles.trainingGrid}><section><h2>{pt ? "Centro de treinamento" : "Training Centre"}</h2><TouchlinePitchSurface ariaLabel="Training squad" className={styles.pitch}><div className={styles.auditCardSpot}><TouchlineEliteExactCard player={player} initialRenderScale={0.27} optimizeForLiveCompact subscribeToRanking={false} enableInteractiveNeon={false} showCardActions={false} showProfileAction={false} showSocialMetrics={false} rankingMode="preview" /></div></TouchlinePitchSurface></section><aside><h3>{pt ? "Treinador" : "Coach"}</h3><TouchlineCoachCard coach={auditCoach} slot={coachSlot} clubName={club.name} countryCode3="ENG" displayMode="compact" optimizeForLiveCompact enableInteractiveNeon={false} /><span>{pt ? "Titulares, reservas e formação" : "Starters, reserves and formation"}</span></aside></div></section>;
...
function AuditArenaPage({ route, language, matchState, persona, player, coachSlot }: { route: TouchlineAuditRoute; language: string; matchState: string; persona: string; player: TouchlineEliteExactPlayer; coachSlot: ReturnType<typeof createTouchlineArenaCoachSlot> }) {
  ...
  if (route.id === "arena/no-round") return <section className={styles.realSurface}>{nav}<div className={styles.noRound}><h2>{pt ? "Nenhuma rodada disponível" : "No matchweek available"}</h2><p>{pt ? "O seu clube e os seus cards permanecem no Centro de Treinamento." : "Your club and cards remain in Training Centre."}</p><button disabled>{pt ? "Ver histórico" : "View history"}</button></div></section>;
